...
| `SPECBANDIT_FALLBACK_PATTERN` | Glob pattern enabling static-split fallback on Redis outage | _(none / disabled)_ |
...
1. When a connection-level Redis error survives the built-in retries, the
   worker globs the pattern locally, sorts the files for determinism, and
   takes the slice where `position % node_total == node_index`.
